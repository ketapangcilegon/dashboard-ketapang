...
async function run() {
  const { data: fsvaMatang } = await supabase.from('fsva_matang').select('*').eq('periode', 2026);
  ...
  const calculatedBorda = skpgMatang.map(item => {
    const fsvaRow = fsvaMatang?.find(x => x.nama_kelurahan === item.nama_kelurahan);
  ...
  console.log("=== Kelurahans with desil <= 4 ===");
  const d1_4 = mapped.filter(x => x.desil <= 4);
  d1_4.forEach(x => {
    console.log(`Rank ${x.rank}: ${x.kelurahan} (Borda Sum: ${x.sum}, Desil: D${x.desil})`);
  });

  console.log("\nTotal items in Desil 1-4:", d1_4.length);
}
