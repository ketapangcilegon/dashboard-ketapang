...
async function scrapeMarket(marketId, name) {
  ...
  try {
    ...
    $('div.card-header').each((_, header) => {
      ...
      while ((match = seriesRegex.exec(scriptText)) !== null) {
        ...
        console.log(`   ${headerText} [${year}]:`, dataArr);
      }
    });
...
async function run() {
  await scrapeMarket('1', 'Pasar Baru Cilegon (Kranggot)');
  await scrapeMarket('2', 'Pasar Kavling Blok F');
  await scrapeMarket('3', 'Pasar Baru Merak');
}
