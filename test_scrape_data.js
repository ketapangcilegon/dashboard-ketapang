const fs = require('fs');
const cheerio = require('cheerio');

async function testScrapeData() {
  process.env.NODE_TLS_REJECT_UNAUTHORIZED = '0';
  
  const body = new URLSearchParams({
    pasar: '1',
    tahun_pertama: '2022',
    tahun_kedua: '2026',
    daterange: '01/01/2022 - 12/31/2026'
  });

  const res = await fetch('https://sagon.cilegon.go.id/infografis/filter', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/x-www-form-urlencoded',
      'User-Agent': 'Mozilla/5.0'
    },
    body
  });

  const html = await res.text();
  const $ = cheerio.load(html);
  
  const result = {};
  
  $('div.card-header').each((_, header) => {
    const headerText = $(header).find('strong').text().trim();
    if (headerText === 'Beras Medium (Cimanuk)') {
      let scriptText = '';
      $(header).parent().find('script').each((_, scr) => {
        const htmlText = $(scr).html() || '';
        if (htmlText.includes('series:')) {
          scriptText = htmlText;
        }
      });
      
      const seriesRegex = /name:\s*['"]([^'"]+)['"],\s*data:\s*\[([^\]]+)\]/g;
      let match;
      while ((match = seriesRegex.exec(scriptText)) !== null) {
        const name = match[1];
        const dataArr = match[2].split(',').map(val => parseInt(val.trim().replace(/['"]/g, ''), 10)).map(val => isNaN(val) ? 0 : val);
        result[name] = dataArr;
      }
    }
  });

  console.log('Beras Medium (Cimanuk) Data:', result);
}

testScrapeData().catch(console.error);
