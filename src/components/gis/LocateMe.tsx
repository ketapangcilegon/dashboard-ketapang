...
export default function LocateMe() {
  ...
  const buildIcon = (heading: number | null) => {
  ...
  useEffect(() => {
    ...
    const ctrl = new LocateBtn({ position: 'topleft' });
    ctrl.addTo(map);
    ...
    const onOrientation = (evt: DeviceOrientationEvent) => {
      ...
      if (anyEvt.webkitCompassHeading !== undefined) {
        heading = anyEvt.webkitCompassHeading;
