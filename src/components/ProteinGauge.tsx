...
export default function ProteinGauge({ value = 63.4 }: ProteinGaugeProps) {
  ...
  if (value >= 57) {
    ...
    statusDesc = 'Masyarakat telah memenuhi atau melebihi batas kecukupan gizi protein harian nasional. Kota Cilegon berada di kategori ini berkat tingginya konsumsi produk hewani (seperti ikan, telur, dan daging) serta makanan jadi di wilayah perkotaan (banten.bps.go.id).';
  ...
  return (
    <div className="flex flex-col h-full bg-white p-4 rounded-xl shadow-sm border border-slate-100 items-center justify-between">
      ...
      <div className="w-full text-left">
        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">Konsumsi Protein</h4>
        <h3 className="text-xs font-bold text-slate-700 mt-1 leading-tight">(gram/kapita/hari)</h3>
      </div>
      ...
      {/* Target info */}
      <p className="text-[9px] text-slate-400 font-bold mt-2">Target Nasional: 57</p>
    </div>
  );
}
